...
**（D）00981A 資料日期整串晚一個交易日——已修正、回補、上線（`main` 01c860b），待生產驗證。**
...
- 其他 22 檔稽核（2026-09-26，收盤價配權重，方法見 `date-alignment.md`）：07-02 ~ 09-24 共
  1,193 組（ETF, 日期）沒有任何一組配到前一日或後一日；15 組判定模糊，全在 07-30／07-31
  （07-31 普通股 11% 漲停、持股價格幾乎等比例，這個方法分不出兩天）。同法在修正前的 DB
  抓到 00981A 08-03 ~ 09-24 全部 39 天「前一日」、修正後全部「當日」。09-21 ~ 09-24 各班 log
  除 00981A 與 00996A 外沒有任何 scraper 走備援。
...
1. **00981A 生產驗證**：下一個交易日 09-29 18:25 主班次，log 應出現 00981A
   `Excel date confirmed` 或 `Excel actual date`、不再有 `falling back to API`，寫入日期為 09-29。
2. **00996A 兆豐**：09-21 起每一班都 `Mega: Failed to open PCF page: HTTP 403`，最新資料停在
   09-18（未處理，要先查是封鎖 GitHub runner IP 還是頁面改版）。
3. **假日產生假報表日**：`main.py` 只避開週末、不認得國定假日；摩根 00401A 在 09-25（中秋）
   把估值日 09-29 夾到請求日寫入，DB 最新日變 09-25，產出一份內容等於 09-24 的報表。
   09-28（教師節）預期再發生一次。未處理。
4. **首頁「各 ETF 近 5 日買賣明細」沒有去重**（`docs/index.html` 的 `loadPerETFData()`）：
   停更或假日時同一次變動在每個報表日重複出現、合計重複計入；另外兩個面板已依
   （ETF, 資料日期）去重。未處理。
5. **上櫃的主動式 ETF 沒被盤點到**：櫃買 09-24 行情有 00411A 主動統一前沿科技、00998A 主動復華
...
8. 觀察（非錯誤）：第一金 00408A／00994A 的權重合計長期在 101–117%，是來源本身的算法。
...
- 驗證層級：00981A 的修正有 red-before／green-after（CI）、回補前後逐筆比對、報表結構化比對、
  收盤價獨立檢驗；尚缺真實班次（09-29）走修好的 Excel 路徑。（A）～（D）各批都**沒有獨立審查**
  （使用者 2026-08-09 決定不做；內建 code review 只在要求時跑，且屬自我審查）。
- 收盤價稽核的腳本沒有進 repo（只有方法寫在 `date-alignment.md`）；上櫃歷史行情要用
