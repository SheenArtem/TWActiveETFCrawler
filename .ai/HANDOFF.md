...
5. **首頁跨報表彙總（原第 5、10 項）——經 PR #1 合併進 main。** 各 ETF 近 5 日買賣明細依（ETF, 資料日期）
   去重、每日欄位歸屬到資料日期；同步買賣超 `modified` 的每日欄位由覆寫改為累加。兩者在 CI 都有
   red-before／green-after（`check_homepage_per_etf_dedup()`、`check_homepage_cross_etf_per_day()`）。
   待辦：在瀏覽器看一次上線的首頁——00408A／00994A／00988A 的變動改畫在資料日期欄、最舊一欄併入
   兩次變動（`date-alignment.md`「已知取捨」）。
