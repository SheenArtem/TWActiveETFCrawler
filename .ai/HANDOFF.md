...
5. **首頁「各 ETF 近 5 日買賣明細」去重——已修，分支 `fix/per-etf-dedup`（已 push），待合併 main。**
   `loadPerETFData()` 比照另外兩個面板依（ETF, 資料日期）去重、每日欄位改歸屬到資料日期（`5736e2d`）。
   `check_homepage_per_etf_dedup()`（node 跑頁內腳本）在 CI 有 red-before／green-after。
   合併要使用者決定；GitHub Pages 發布 `main` 的 `docs/`，合併後才上線，上線後在瀏覽器看一次。
   目前視窗無重複群組，合計不變；00408A／00994A／00988A 改畫在資料日期欄、最舊一欄併入兩次變動
   （`date-alignment.md`「已知取捨」）。
...
10. 同步買賣超（`loadCrossETFData()`）的 `modified` 對每日欄位是覆寫不是累加（`docs/index.html:1153`）：
    同一欄收到兩次變動時（落後來源的最舊一欄）只顯示後處理那筆、合計兩筆都算；09-24 視窗有 4 組
    （ETF, 股票）符合。未處理。
