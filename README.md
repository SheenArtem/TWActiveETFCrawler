...
- ✅ **多家投信支援**：EZMoney、野村、群益、復華、中信、第一金、台新、安聯、國泰、摩根、富邦、聯博、兆豐、凱基、永豐
- ✅ **每日自動抓取**：主觸發是 Cloudflare Workers Cron（台北 18:25），GitHub Actions 另有較晚的後備班次（cron 設 UTC 13:00，實測實際落在台北 22:21–00:25）
- ✅ **變動追蹤**：自動偵測並報告成分股變動（新增/移除/持股變化）
- ✅ **持股單位顯示**：以台灣習慣的「張」為單位（1張 = 1000股）；海外成分股標「千股」
...
目前追蹤 **23 檔**：上市的 22 檔純台股主動式 ETF 全部，加上 1 檔全球型 00988A 主動統一全球創新
（2026-09-03 加入；其餘海外股票型、債券型不在範圍）。00988A 約四分之三權重是海外股票，
成分股代號沿用來源的 Bloomberg 格式「代號 市場」（如 `SNDK US`、`6981 JP`），報表上以「千股」計
（台股仍是「張」，兩者都是 1000 股）。

ETF 名稱一律採**證交所官方簡稱**，唯一來源是 [`src/etf_names.py`](src/etf_names.py)；
下表若與該檔不一致，以該檔為準。

| 投信 | CLI 參數 | 追蹤的 ETF |
|------|----------|------------|
| 統一投信 (EZMoney) | `--ezmoney` | 00981A 主動統一台股增長、00403A 主動統一升級50、00988A 主動統一全球創新（全球型，含海外成分股） |
| 野村投信 | `--nomura` | 00980A 主動野村臺灣優選、00985A 主動野村台灣50、00999A 主動野村臺灣高息 |
| 群益投信 | `--capital` | 00982A 主動群益台灣強棒、00992A 主動群益科技創新 |
| 復華投信 | `--fhtrust` | 00991A 主動復華未來50 |
| 中信投信 | `--ctbc` | 00995A 主動中信台灣卓越、00406A 主動中信台灣收益 |
| 第一金投信 | `--fsitc` | 00994A 主動第一金台股優、00408A 主動第一金優股息 |
| 台新投信 | `--tsit` | 00987A 主動台新優勢成長 |
| 安聯投信 | `--allianz` | 00984A 主動安聯台灣高息、00993A 主動安聯台灣 |
| 國泰投信 | `--cathay` | 00400A 主動國泰動能高息 |
| 摩根投信 | `--morgan` | 00401A 主動摩根台灣鑫收 |
| 富邦投信 | `--fubon` | 00405A 主動富邦台灣龍耀 |
| 聯博投信 | `--abfunds` | 00404A 主動聯博動能50 |
| 兆豐投信 | `--megafunds` | 00996A 主動兆豐台灣豐收 |
| 凱基投信 | `--kgi` | 00407A 主動凱基台灣 |
| 永豐投信 | `--sinopac` | 00410A 主動永豐科技趨勢 |

各投信的資料取得方式（下載檔案／網頁 DOM／API）、資料日期來源與已知陷阱，
整理在 [`.ai/guides/data-sources.md`](.ai/guides/data-sources.md)。
...
# 只更新特定投信（完整參數見上表或 python main.py --help）
...
│   ├── stock_markets.py        # 成分股代號慣例（台股／海外 Bloomberg 代號）與報表單位
...
│   ├── megafunds_scraper.py    # 兆豐投信爬蟲（申購買回清單 WebForms postback）
│   ├── kgi_scraper.py          # 凱基投信爬蟲（申購買回清單 partial view）
│   ├── sinopac_scraper.py      # 永豐投信爬蟲（現金申購買回清單 SSR）
│   └── utils.py                # 工具函數
├── data/
