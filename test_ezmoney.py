...
def test_date_conversion():
    ...
    for date_str in test_cases:
        roc_date = scraper._convert_to_roc_date(date_str)
        back_to_ad = scraper._convert_from_roc_date(roc_date)
        print(f"西元: {date_str} -> 民國: {roc_date} -> 西元: {back_to_ad}")
        assert date_str == back_to_ad, "日期轉換錯誤！"
...
def test_etf_mapping():
    ...
    # 測試現有對照
    fund_code = scraper.get_fund_code('00981A')
    print(f"00981A -> {fund_code}")
    assert fund_code == '49YTW', "ETF 代碼對照錯誤！"
    
    # 測試新增對照
    scraper.add_etf_mapping('00000X', 'TEST123')
    fund_code = scraper.get_fund_code('00000X')
    print(f"00000X -> {fund_code}")
    assert fund_code == 'TEST123', "新增對照失敗！"
...
def test_get_holdings():
    """測試抓取 00981A 持股明細"""
    print("\n" + "=" * 70)
    print("測試抓取 00981A 持股明細")
    print("=" * 70)
    
    scraper = EZMoneyScraper()
    
    # 使用今天的日期（瀏覽器測試確認 2026-01-26 有數據）
    ...
    print("\n前 10 大持股:")
    print("-" * 70)
    print(f"{'股票代號':<10} {'股票名稱':<15} {'持股數':<15} {'權重(%)':<10} {'市值':<15}")
    print("-" * 70)
    
    for i, holding in enumerate(holdings[:10], 1):
        print(f"{holding['stock_code']:<10} "
              f"{holding['stock_name']:<15} "
              f"{holding['shares']:>15,} "
              f"{holding['weight']:>9.2f} "
              f"{holding.get('market_value', 0):>15,.0f}")
...
def test_save_to_database(holdings):
    ...
    try:
        ...
        # 查詢驗證
        latest_date = db.get_latest_date('00981A')
        print(f"資料庫中 00981A 的最新日期: {latest_date}")
